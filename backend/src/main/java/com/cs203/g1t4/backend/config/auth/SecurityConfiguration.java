...
public class SecurityConfiguration {
    ...
    public static final String[] whiteListedRoutes = new String[] {
        ...
        "/api/event/exploreEvent/.*",
        "/api/event/detailsEvent/.*",
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        return http.cors(withDefaults())
                // Disables CSRF (Cross-Site Request Forgery) protection. Deprecated and can be
                // removed
                .csrf(csrf -> csrf.disable())
                // Configure authorization rules for your application. Include all
                // requestMatchers within "{}"
                ...
                // Treats each request independently, and there is no server-side session state
                // stored between requests.
                .sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                // Authentication provider is responsible for authenticating users based on
                // their credentials.
                .authenticationProvider(authenticationProvider)
                // Adds custom JWT authentication filter (jwtAuthFilter) before the default
